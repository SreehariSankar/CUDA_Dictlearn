...
for lll in range(10):
    ...
    time_start = timeit.timeit()
    update_dict=admm.bpdn.BPDN(u,data_ra,-0.6)

    # update_dict=cmod.CnstrMOD(cp2np(u[p]),cp2np(data_ra[p].T),v_ra.shape)
    v_ra=update_dict.solve()
        # np2cp(update_dict.getdict())
    time_stop = timeit.timeit()
    print(time_stop - time_start,lll)
    # for p in u:
    #     A0+=p@p.T
    #     B0+=u_ra@s
    #     # FEATURES X NEURONS
    ...
    print(cp.linalg.norm(v_ra.T@u-data_ra.T))
